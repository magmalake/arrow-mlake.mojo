...
`assert_preorder` is the half a lockstep walk cannot see: a check that starts
at the roots and follows child indices folds a consistently permuted arena to
the same answer, because it never looks at the numbers themselves. So the
import's arena layout is asserted directly — the nodes of an imported tree are
its pre-order DFS, `root`, `root + 1`, … with nothing else in between — and
`permuted_arena` is the negative control that renumbers an arena without
touching a value, so a test can show that `assert_same_array` is blind to it
and `assert_preorder` is not.
