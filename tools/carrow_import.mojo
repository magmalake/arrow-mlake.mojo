...
`tools/produce_c_data.py` has pyarrow build an array, calls `_export_to_c` on
it, and passes the two addresses in here — so the producer under test is one we
did not write, and a format string or buffer convention we got subtly wrong
shows up as a wrong value rather than as two of our own modules agreeing with
each other. `am_import_roundtrip` then exports what it imported, so pyarrow
compares what comes back with what it sent and the *export* side is gated by
the same run.
...
```console
mojo build --emit shared-lib tools/carrow_import.mojo -I src -o build/libamcimport.so
```
"""
...
@export("am_import_describe")
def am_import_describe(
    array: Int,
    schema: Int,
...
@export("am_import_roundtrip")
def am_import_roundtrip(
    array: Int,
    schema: Int,
...
@export("am_stream_describe")
def am_stream_describe(
    stream: Int,
    buf: Pointer[UInt8, MutUntrackedOrigin],
    buflen: Int,
) abi("C") -> Int32:
