...
An import checked only against our own export would be two of our modules
...
* `am_import_describe` reports the type as *we* would spell it back out, plus
  the length, the null count and the child count. A format string we misread
  fails here, loudly, with both spellings visible.
* `am_import_roundtrip` imports and re-exports, and pyarrow compares what
...
`am_stream_describe`.
    ...
    python tools/produce_c_data.py build/libamcimport.dylib
"""
...
LIB = sys.argv[1] if len(sys.argv) > 1 else "build/libamcimport.dylib"

lib = ctypes.CDLL(LIB)
lib.am_import_describe.restype = ctypes.c_int32
lib.am_import_describe.argtypes = [
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_int64,
]
lib.am_import_roundtrip.restype = ctypes.c_int32
lib.am_import_roundtrip.argtypes = [
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_void_p,
]
lib.am_stream_describe.restype = ctypes.c_int32
lib.am_stream_describe.argtypes = [
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_int64,
]
...
buf = ctypes.create_string_buffer(4096)
rc = lib.am_stream_describe(ctypes.addressof(stream), buf, len(buf))
...
buf = ctypes.create_string_buffer(4096)
rc = lib.am_import_describe(ctypes.addressof(a), ctypes.addressof(s), buf, len(buf))
text = buf.value.decode("utf-8", "replace")
