...
`arrow_mlake.carrow` is the outbound half: an `ArrayData` tree becomes a C
...
callback alive behind it — would be zero-copy but changes the type every
consumer of `ArrayData` reads, so it is deliberately not what this does. On a
16-column, 65k-row batch the copy runs at memory-copy speed, which is a
rounding error next to decoding the same data out of a file format.
...
def extension_name(metadata: Int) raises -> String:
    ...
    if n < 0 or n > _MAX_METADATA_KEYS:
        raise Error(
            String("arrow_mlake.carrow: implausible metadata key count ", n)
        )
    var at = 4
    for _ in range(n):
        ...
        if klen < 0 or klen > _MAX_METADATA_LEN:
            raise Error(
                String(
                    "arrow_mlake.carrow: implausible metadata key length ", klen
                )
            )
        var key_at = at + 4
        at = key_at + klen
        ...
        if vlen < 0 or vlen > _MAX_METADATA_LEN:
            raise Error(
                String(
                    "arrow_mlake.carrow: implausible metadata value length ",
                    vlen,
                )
            )
        var val_at = at + 4
...
def _check(cond: Bool, message: StringSlice) raises:
    if not cond:
        raise Error(String("arrow_mlake.carrow: ", message))
...
def _import_node(
    ...
    first subtree, then the next — which is the order `arrow_mlake.carrow`'s
...
struct ImportedStream(Movable):
    ...
    def __init__(out self, stream: Int) raises:
        ...
        if s[].release == 0:
            self._schema = 0
            raise Error(
                "arrow_mlake.carrow: the producer handed over a released"
                " ArrowArrayStream"
            )
        ...
        if rc != 0:
            why = String(
                "arrow_mlake.carrow: ArrowArrayStream get_schema failed (",
                rc,
                "): ",
            ...
            why = String(
                "arrow_mlake.carrow: ArrowArrayStream get_schema returned"
                " nothing"
            )
        if why:
    ...
    def next(mut self) raises -> Optional[RecordBatch]:
        ...
        if rc != 0:
            ...
            raise Error(
                String(
                    "arrow_mlake.carrow: ArrowArrayStream get_next failed (",
                    rc,
                    "): ",
